             ...
             bool& fIsEncrypted,  bool& fAnyUnordered, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "nameBlockNot")
        {
            ...
            ssKey >> strAddress;
            ssValue >> strvalue;
            if(strvalue.find_first_of('+',0) == string::npos)
                pwallet->mapNamesBookDoNotRegister[CQcoinAddress(strAddress).Get()] = strvalue;
            else
                pwallet->mapNamesBookDoNotRegister[CQcoinAddress(strAddress).Get()] = CQcoinAddress(strAddress).ToString();
        }
        else if (strType == "tx")
        {
            uint256 hash;
            ...
            //// debug print
            //logPrint("LoadWallet  %s\n", wtx.GetHash().ToString().c_str());
            //logPrint(" %12"PRI64d"  %s  %s  %s\n",
            //    wtx.vout[0].nValue,
        ...
        else if (strType == "key" || strType == "wkey")
        {
            ...
            if (strType == "key")
            {
                ...
                {
                    strErr = "Error reading root.database: CPrivKey corrupt";
                    return false;
                }
                if (key.GetPubKey() != vchPubKey)
                {
                    strErr = "Error reading root.database: CPrivKey pubkey inconsistency";
                    return false;
                }
                if (!key.IsValid())
                {
                    strErr = "Error reading root.database: invalid CPrivKey";
                    return false;
                }
            }
            else
            {
                ...
                if (key.GetPubKey() != vchPubKey)
                {
                    strErr = "Error reading root.database: CWalletKey pubkey inconsistency";
                    return false;
                }
                if (!key.IsValid())
                {
                    strErr = "Error reading root.database: invalid CWalletKey";
                    return false;
                }
            }
            if (!pwallet->LoadKey(key))
            {
                strErr = "Error reading root.database: LoadKey failed";
                return false;
            }
        }
        else if (strType == "mkey")
        {
            ...
            {
                strErr = strprintf("Error reading root.database: duplicate CMasterKey id %u", nID);
                return false;
            }
        ...
        else if (strType == "ckey")
        {
            ...
            {
                strErr = "Error reading root.database: LoadCryptedKey failed";
                return false;
            }
            fIsEncrypted = true;
        }
        ...
        else if (strType == "cscript")
        {
            ...
            {
                strErr = "Error reading root.database: LoadCScript failed";
                return false;
            }
        }
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        ...
        if (!pcursor)
        {
            logPrint("Error getting root.database cursor\n");
            return DB_CORRUPT;
        }
        ...
        loop
        {
            ...
            {
                logPrint("Error reading next record from root.database\n");
                return DB_CORRUPT;
            }
    ...
    logPrint("nFileVersion = %d\n", nFileVersion);

    BOOST_FOREACH(uint256 hash, vWalletUpgrade)
        WriteTx(hash, pwallet->mapWallet[hash]);

    // Rewrite encrypted wallets of versions 0.4.0 and 0.5.0rc:
    if (fIsEncrypted && (nFileVersion == 00001 || nFileVersion == 00001))
        ...
        if (nLastFlushed != nWalletDBUpdated && GetTime() - nLastWalletUpdate >= 2)
        {
            ...
            if (lockDb)
            {
                ...
                if (nRefCount == 0)
                {
                    ...
                    {
                        logPrint("Flushing myq.dat\n");
                        nLastFlushed = nWalletDBUpdated;
                        int64 nStart = GetTimeMillis();

                        // Flush myq.dat so it's self contained
                        bitdb.CloseDb(strFile);
                        bitdb.CheckpointLSN(strFile);

                        bitdb.mapFileUseCount.erase(mi++);
                        logPrint("Flushed myq.dat %"PRI64d"ms\n", GetTimeMillis() - nStart);
                    }
                }
            }
        }
...
#endif
                    logPrint("copied myq.dat to %s\n", pathDest.string().c_str());
                    return true;
                } catch(const filesystem::filesystem_error &e) {
                    logPrint("error copying myq.dat to %s - %s\n", pathDest.string().c_str(), e.what());
                    return false;
                }
            }
        }
...
//
// Try to (very carefully!) recover myq.dat if there is a problem.
//
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    // Recovery procedure:
    // move myq.dat to wallet.timestamp.bak
    // Call Salvage with fAggressive=true to
    // get as much data as possible.
    // Rewrite salvaged data to myq.dat
    ...
    if (result == 0)
        logPrint("Renamed %s to %s\n", filename.c_str(), newFilename.c_str());
    else
    {
        logPrint("Failed to rename %s to %s\n", filename.c_str(), newFilename.c_str());
        return false;
    }
    ...
    {
        logPrint("Salvage(aggressive) found no records in %s.\n", newFilename.c_str());
        return false;
    }
    logPrint("Salvage(aggressive) found %"PRIszu" records\n", salvagedData.size());
    ...
    BOOST_FOREACH(CDBEnv::KeyValPair& row, salvagedData)
    {
        if (fOnlyKeys)
        {
            ...
            if (!fReadOK)
            {
                logPrint("WARNING: CWalletDB::Recover skipping %s: %s\n", strType.c_str(), strErr.c_str());
                continue;
            }
        }
