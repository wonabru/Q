...
const int Q_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString Q_IPC_PREFIX("qcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(Q_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(Q_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(Q_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click qcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start qcoin: click-to-pay handler");
    else
...
{
    // clicking on qcoin: URLs creates FileOpen events on the Mac:
