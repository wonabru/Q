...
static const char* pszBase58 = "Q123456789ABCDEFGHJKLMNPRSTUVWXYZabcdefghijkmnopqrstuvwxyz";

// Encode a byte sequence as a base58-encoded string
inline std::string EncodeBase58(const unsigned char* pbegin, const unsigned char* pend)
{
    std::string ret="";
    std::vector<unsigned char> *vchtemp = new std::vector<unsigned char>();
    vchtemp->assign(pbegin,pend);
    CBigNum x,y;
    x.setvch((*vchtemp));
    while(x>0)
    {
...
inline bool DecodeBase58(const char* psz, std::vector<unsigned char>& vchRet)
{
    ...
    while(*psz!='\0')
    {
       ret += ret * 58 + (unsigned)(*(pszBase58 + (unsigned char)(*psz)));
       psz++;
}
    vchRet = ret.getvch();
    return true;
