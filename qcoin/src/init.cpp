...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Mark version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  qcoind [options]                     " + "\n" +
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "\n" + _("SSL options: (see the Mark Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Make sure only a single Mark process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Mark is probably already running."), strDataDir.c_str()));
