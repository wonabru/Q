...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemaphoreGrant *grantOutbound = NULL, const char *strDest = NULL, bool fOneShot = true);
...
bool GetMyExternalIP2(const CService& addrConnect, const char* pszGet, const char* pszKeyword, CNetAddr& ipRet)
{
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty()) // HTTP response is separated from headers by blank line
        {
            ...
            logPrint("GetMyExternalIP() received [%s] %s\n", strLine.c_str(), addr.ToString().c_str());
...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("qcoin-ext-ip");

    CNetAddr addrLocalHost;
    if (GetMyExternalIP(addrLocalHost))
    {
        logPrint("GetMyExternalIP() returned %s\n", addrLocalHost.ToStringIP().c_str());
...
void CNode::PushVersion()
{
    ...
    logPrint("send version message: version %d, blocks=%d, us=%s, them=%s, peer=%s\n", PROTOCOL_VERSION, nBestHeight, addrMe.ToString().c_str(), addrYou.ToString().c_str(), addr.ToString().c_str());
...
bool CNode::Misbehaving(int howmuch)
{
    ...
    if (nMisbehavior >= GetArg("-banscore", 100))
    {
        ...
        logPrint("Misbehaving: %s (%d -> %d) DISCONNECTING\n", addr.ToString().c_str(), nMisbehavior-howmuch, nMisbehavior);
        {
            LOCK(cs_setBanned);
...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        if (nSelect == SOCKET_ERROR)
        {
            ...
            {
                int nErr = WSAGetLastError();
                logPrint("socket select error %d\n", nErr);
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            if (hSocket != INVALID_SOCKET)
                if (!addr.SetSockAddr((const struct sockaddr*)&sockaddr))
                    logPrint("Warning: Unknown socket family\n");
            ...
            if (hSocket == INVALID_SOCKET)
            {
                ...
                if (nErr != WSAEWOULDBLOCK)
                    logPrint("socket error accept failed: %d\n", nErr);
            }
            ...
            {
                logPrint("connection from %s dropped (banned)\n", addr.ToString().c_str());
                closesocket(hSocket);
            }
            else
            {
                logPrint("accepted connection %s\n", addr.ToString().c_str());
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (GetTime() - pnode->nTimeConnected > 60)
            {
                ...
                {
                    logPrint("socket no message in first 60 seconds, %d %d\n", pnode->nLastRecv != 0, pnode->nLastSend != 0);
                    pnode->fDisconnect = true;
                }
                else if (GetTime() - pnode->nLastSend > 90*60 && GetTime() - pnode->nLastSendEmpty > 90*60)
                {
                    logPrint("socket not sending\n");
                    pnode->fDisconnect = true;
                }
                else if (GetTime() - pnode->nLastRecv > 90*60)
                {
                    logPrint("socket inactivity timeout\n");
                    pnode->fDisconnect = true;
                }
            }
        }
    ...
    if (r == 1)
    {
        if (fDiscover) {
            ...
            if(r != UPNPCOMMAND_SUCCESS)
                logPrint("UPnP: GetExternalIPAddress() returned %d\n", r);
            else
            {
                if(externalIPAddress[0])
                {
                    logPrint("UPnP: ExternalIPAddress = %s\n", externalIPAddress);
        ...
        string strDesc = "Mark " + FormatFullVersion();
        ...
        try {
            loop {
                ...
                if(r!=UPNPCOMMAND_SUCCESS)
                    logPrint("AddPortMapping(%s, %s, %s) failed with code %d (%s)\n",
                        port.c_str(), port.c_str(), lanaddr, r, strupnperror(r));
                else
                    logPrint("UPnP Port Mapping successful.\n");;
        ...
        catch (boost::thread_interrupted)
        {
            ...
            logPrint("UPNP_DeletePortMapping() returned : %d\n", r);
            freeUPNPDevlist(devlist); devlist = 0;
            FreeUPNPUrls(&urls);
            throw;
        }
    } else {
        logPrint("No valid UPnP IGDs found\n");
        freeUPNPDevlist(devlist); devlist = 0;
        if (r != 0)
            FreeUPNPUrls(&urls);
    }
...
static const char *strMainNetDNSSeed[][2] = {
    {"qcoin.sipa.be", "seed.qcoin.sipa.be"},
    {"bluematt.me", "dnsseed.bluematt.me"},
    {"dashjr.org", "dnsseed.qcoin.dashjr.org"},
    {"xf2.org", "bitseed.xf2.org"},
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
    {"qcoin.petertodd.org", "testnet-seed.qcoin.petertodd.org"},
...
void ThreadDNSAddressSeed()
{
    ...
    logPrint("Loading addresses from DNS seeds (could take a while)\n");
    ...
    logPrint("%d addresses found from DNS seeds\n", found);
}
...
void DumpAddresses()
{
    ...
    logPrint("Flushed %d addresses to peers.dat  %"PRI64d"ms\n",
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Mark is probably already running."), addrBind.ToString().c_str());
        else
        ...
        BOOST_FOREACH(SOCKET hListenSocket, vhListenSocket)
            if (hListenSocket != INVALID_SOCKET)
                if (closesocket(hListenSocket) == SOCKET_ERROR)
                    logPrint("closesocket(hListenSocket) failed with error %d\n", WSAGetLastError());
