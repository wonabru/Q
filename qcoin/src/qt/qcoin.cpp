...
{
    return QCoreApplication::translate("qcoin-core", psz).toStdString();
}
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", QcoinGUI::tr("A fatal error occurred. Marka can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        ...
        // (which not yet possible because lang=XX can be overridden in qcoin.conf in the data directory)
        QMessageBox::critical(0, "Marka",
    ...
    QApplication::setOrganizationName("Marka");
    ...
    // Get desired locale (e.g. "pl_PL") from command line or use system locale
    QString lang_territory = QString::fromStdString(GetArg("-lang", QLocale::system().name().toStdString()));
    QString lang = lang_territory;
    // Convert to "pl" only by truncating "_PL"
    ...
    // Load e.g. qt_pl_PL.qm
    if (qtTranslator.load("qt_" + lang_territory, QLibraryInfo::location(QLibraryInfo::TranslationsPath)))
        app.installTranslator(&qtTranslator);

    // Load e.g. qcoin_pl.qm (shortcut "pl" needs to be defined in qcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        app.installTranslator(&translatorBase);

    // Load e.g. qcoin_pl_PL.qm (shortcut "pl_PL" needs to be defined in qcoin.qrc)
