...
inline std::string EncodeBaseQ(const unsigned char* pbegin, const unsigned char* pend)
{
    ...
    // Expected size increase from base58 conversion is approximately 137%
    // use 138% to be safe
    str.reserve((pend - pbegin) * 200 / 100 + 1);
    CBigNum dv;
    CBigNum rem;
    while (bn > bn0)
    {
...
// Encode a byte vector as a base58-encoded string
inline std::string EncodeBaseQ(const std::vector<unsigned char>& vch)
{
    return EncodeBaseQ(&vch[0], &vch[0] + vch.size());
}
/*
// Decode a base58-encoded string psz into byte vector vchRet
// returns true if decoding is successful
