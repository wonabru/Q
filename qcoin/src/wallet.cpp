...
bool CWallet::EncryptWallet(const SecureString& strWalletPassphrase)
{
    ...
    logPrint("Encrypting Wallet with an nDeriveIterations of %i\n", kMasterKey.nDeriveIterations);
...
void CWallet::WalletUpdateSpent(const CTransaction &tx)
{
    ...
    // Update the wallet spent flag if it doesn't know due to myq.dat being
    // restored from backup or the user making copies of myq.dat.
    {
        LOCK(cs_wallet);
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            {
                // Update fSpent if a tx got spent somewhere else by a copy of myq.dat
...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of qcoin before 0.8.6 had a bug that inserted
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    CPubKey vchPubKey = vchDefaultKey;
...
string CWallet::SendMoney(CKeyID scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    if (!CommitTransaction(wtxNew, reservekey))
        return _("Error: The transaction was rejected! This might happen if some of the coins in your wallet were already spent, such as if you used a copy of myq.dat and coins were spent in the copy but not marked as spent here.");
...
bool CWallet::SetNameBookRegistered(const CTxDestination& address, const string& strName, int ato)
{
    ...
    bool ret = CWalletDB(strWalletFile).WriteNameBlock(CQcoinAddress(address).ToString(), strName);
...
bool CWallet::SetNameBookNotToRegistered(const CTxDestination& address, const string& strName, int ato)
{
    ...
    bool ret = CWalletDB(strWalletFile).WriteNameBlockDoNotRegister(CQcoinAddress(address).ToString(), name);
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    bool ret = CWalletDB(strWalletFile).EraseName(addr.ToString());
...
{
    {
        ...
        int64 nKeys = max(GetArg("-keypool", 3), (int64)0);
...
{
    {
        ...
        // Top up key pool
        unsigned int nTargetSize = max(GetArg("-keypool", 3), 0LL);
