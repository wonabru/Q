...
To build PLM (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Q/.git/
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/PLM-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/PLM-build.assert
```
...
[PLM/gitian.sigs](https://github.com/wonabru/Q/gitian.sigs/) repository, or
if not possible create a pull request. You can also mail the files to me
(q@wonabru.com) and I'll commit them.
